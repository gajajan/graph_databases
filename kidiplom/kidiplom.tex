...
\begin{listing}[h]
\begin{minted}{Groovy}
g.V().has("Destination", "destinationId", 7).inE("LED_TO").
  ...
  order().
    by(__.select("rating").
    map { it.get() instanceof Number ? 0 : 1 }).
...
\item[\texttt{ArangoDB-docker/}] \hfill \\
Adresář obsahující potřebné soubory pro sestavení kontejneru v Dockeru pro databázi ArangoDB. Do kontejneru jsou též zkopírovány datasety zmiňované v práci, na kterých lze zkoušet dotazy v dotazovacím jazyku AQL z adresáře \texttt{/queries}.

\item[\texttt{images/}] \hfill \\
Adresář obsahující obrázky, které jsou použité v \texttt{readme.txt}.

\item[\texttt{kidiplom/}] \hfill \\
...
\item[\texttt{Neo4j-docker/}] \hfill \\
Adresář obsahující potřebné soubory pro sestavení kontejneru v Dockeru pro databázi Neo4j. Do kontejneru jsou též zkopírovány datasety zmiňované v práci, na kterých lze zkoušet dotazy v dotazovacím jazyku Cypher z adresáře \texttt{/queries}.

\item[\texttt{OrientDB-docker/}] \hfill \\
Adresář obsahující potřebné soubory pro sestavení kontejneru v Dockeru pro databázi OrientDB. Do kontejneru jsou též zkopírovány datasety zmiňované v práci, na kterých lze zkoušet dotazy v dotazovacím jazyku Gremlin nebo SQL z adresáře \texttt{/queries}.

\item[\texttt{queries/}] \hfill \\
Adresář obsahuje podadresáře určené pro vybrané implementace grafových databází. V těchto poadresářích jsou textové soubory s ukázkovými dotazy. 

\item[\texttt{Measurement.xlsx}] \hfill \\
