...
V této podkapitole jsou ukázány tři zdrojové kódy \ref{kod:recommendation}, \ref{kod:substring} a \ref{kod:favouriteMonth} pro dotazy odpovídající na otázky z podkapitoly \ref{sec:problem-definition}. Výsledky dotazů jsou v tabulkách \ref{tab:recommendation}, \ref{tab:substring} a \ref{tab:favouriteMonth}. U dotazů, které vracejí více než tři výsledky, jsou uvedeny pouze první tři. Desetinná čísla ve výsledcích jsou zaokrouhlena na 3 desetinná místa.
...
\begin{table}[h]
\begin{center}
\caption[Tabulka výsledků]{Výsledná tabulka pro dotaz ve zdrojovém kódu \ref{kod:recommendation}.}
...
\begin{table}[h]
\begin{center}
\caption[Tabulka výsledků]{Výsledná tabulka pro dotaz ve zdrojovém kódu \ref{kod:substring}.}
\label{tab:substring}
\begin{tabular}{ | l | c | }
 \hline
...
\begin{table}[h]
\begin{center}
\caption[Tabulka výsledků]{Výsledná tabulka pro dotaz ve zdrojovém kódu \ref{kod:favouriteMonth}.}
\label{tab:favouriteMonth}
\begin{tabular}{ | l | c | }
 \hline
 \textbf{month}  & \textbf{averageRating} \\
 \hline \hline
 5 & 3,25 \\
 3 & 2,625 \\
 1 & not rated \\
...
Pro hledání nejkratší cesty v ohodnoceném grafu se nejčastěji využívá Djikstrův algoritmus. Ve zdrojovém kódu \ref{kod:ShortestPath-Weighted-Example} z knihy \cite{practitioner} je ukázána verze Djikstrova algoritmu pro hledání cesty mezi dvěma vrcholy napsaná v jazyce Gremlin. Dotaz najde nejkratší cestu mezi dvěma vrcholy v ohodnoceném grafu a vrátí jména stanic na této cestě, jejich počet a délku této cesty. Djikstrův algoritmus je více popsán v následující kapitole. 
...
Pro testování byl použit dataset \cite{youtubeDataset} obsahující informace o vztazích uživatelů ze sociální sítě Youtube. Pro testování byl též vytvořen csv soubor obsahující identifikátory všech uživatelů v síti a další soubor s vlastnostmi. Tyto data tvoří neorientovaný graf. Při testování byly použity orientované hrany a obecně byly procházeny jak výchozí, tak příchozí hrany. Příslušný logický datový model k testovanému datasetu ukazuje obrázek \ref{fig:Youtube-Graph}. Seznam souborů shrnuje tabulka \ref{tab:Youtube-csv}.
