...
\thanks{Děkuji své rodině za podporu a panu Mgr. Tomáši Urbancovi, Ph.D., za vedení a cenné rady při psaní této práce.}
...
Grafy se dělí na \textit{orientované} a \textit{neorientované}. Hrana v neorientovaném grafu spojující vrcholy $u$ a $v$ se nazývá \textit{neorientovaná} a zapisuje se pomocí množiny obsahující dvojici vrcholů, tedy $ \{ u, v \} $. Hrana v orientovaném grafu začínající ve vrcholu $u$ a končící ve vrcholu $v$ se nazývá \textit{orientovaná} a pro zápis se používá uspořádaná dvojice, tedy $\langle u, v \rangle$. O orientované hraně začínající i končící ve stejném vrcholu mluvíme jako o \textit{smyčce}. Hrany grafu nazveme \textit{vícenásobné}, pokud existuje mezi dvěma vrcholy hran více, jinak jsou \textit{jednoduché}. Grafu obsahujícímu vícenásobné hrany říkáme \textit{multigraf}.

Grafy se nejčastěji znázorňují pomocí kroužků, které představují vrcholy, a úseček mezi nimi, které představují hrany. Pokud se jedná o orientovaný graf místo úseček používáme šipky.
...
Symetrizaci grafu lze popsat jako proces, ve kterém zapomeneme orientaci hran, odstraníme smyčky a vícenásobné hrany nahradíme jednoduchými.
...
Řekneme, že vrcholy $u,v \in V$ v grafu $G = \langle V,E\rangle$ spolu \textit{sousedí}, pokud mezi nimi existuje hrana  $e \in E$. Říkáme, že vrcholy $u,v$ jsou \textit{incidentní} s $e$. \textit{Sousedství} vrcholu $v \in V$ je množina vrcholů $N(v)$ obsahující všechny vrcholy, se kterými vrchol $v$ sousedí.
\end{definition}
...
\begin{center}
$ deg_{in}(u) = | \{ \langle v, u \rangle \mid v \in V \} | $
\end{center}
a \textit{výstupní stupeň vrcholu $u$} definovaný jako 
\begin{center}
$ deg_{out}(u) = | \{ \langle u, v \rangle \mid v \in V \} | $.
\end{center}
...
\textit{Cesta} je sled, ve kterém pro každé dva vrcholy $v_i$ a $v_j$ platí: $v_i \neq v_j$, právě když $i \neq j$. V neohodnocených grafech má cesta obsahující $n+1$ vrcholů \textit{délku} $n$. Délka cesty v ohodnoceném grafu je rovna součtu ohodnocení hran na cestě. Pro vrcholy $u,v$ grafu $G$ definujeme \textit{vzdálenost} vrcholu $u$ a vrcholu $v$ jako délku cesty z $u$ do $v$, která je ze všech délek cest z $u$ do $v$ nejmenší. Značíme ji $dist(u,v)$.

\textit{Kružnice} je cesta, kde počáteční a koncový vrchol je stejný. U orientovaného grafu používáme pojem \textit{cyklus}. O grafu $G = \langle V,E\rangle$ řekneme, že je \textit{cyklický}, pokud se v grafu vyskytují kružnice, nebo cykly. Jinak graf $G$ nazveme \textit{acyklický}.
...
Graf $G = \langle V,E\rangle$ nazýváme \textit{souvislý}, pokud pro každé dva vrcholy $ u,v \in V $ platí, že existuje cesta z $u$ do $v$. Maximální souvislý podgraf grafu nazýváme \textit{komponenta grafu}.
\end{definition}

U orientovaného grafu $H = \langle V_H,E_H \rangle$ rozlišujeme slabou a silnou souvislost. Graf H je \textit{slabě souvislý}, pokud jeho symetrizace je souvislý graf. O grafu H řekneme, že je \textit{silně souvislý}, pokud pro každé dva vrcholy $ u,v \in V_H $ platí, že existuje cesta z $u$ do $v$.
...
Při psaní této podkapitoly jsem čerpal též ze zdrojů \cite{indexFreeAdjacency} a \cite{neo4jNative}. Grafové databáze můžeme mezi sebou rozlišovat podle zpracování a ukládání dat. Grafová databáze má \textit{nativní grafové uložiště}, pokud umožňuje uchovávání a správu dat v grafové podobě. Tento přístup zefektivňuje práci s databází, neboť uložiště je optimalizované přímo pro tento typ dat. Nicméně ne všechny grafové databáze mají grafové uložiště. Některé z nich tato data serializují do jiných typů databází. Tento způsob se především opírá o vyspělost a známost jiných technologií.
...
Grafové databáze, implementující nativní grafové uložiště a index-free adjacency označujeme jako \textit{nativní}. Pracují efektivněji při průchodu grafem. Pro práci s velkým množstvím hustě propojených dat je tato architektura nezbytná. Nenativní grafové databáze jsou bezpečnější volbou pro malé projekty a ne příliš propojená data, neboť obvykle staví na technologiích, které jsou známé a lety ověřené.
...
Tento model je zkráceně označován jako DAG. Grafy implementující DAG model jsou acyklické a orientované. Tento model je vhodný pro plánování úkolů, zpracování dat, nebo systémy pro správu verzí.
...
\subsubsection{Neo4j}
Při psaní této podkapitoly jsem čerpal též ze zdrojů \cite{neo4jDoc} a \cite{neo4jDev}. Databázový systém Neo4j je nejrozšířenější open source grafovou databází na trhu s největší uživatelskou základnou. Nabízí vysoký výkon při práci s grafy. Její první verze byla vyvinuta v roce 2002 a vznikla z důvodu nespokojenosti jejich zakladatelů s výkonem relačních databází. Implementována je v Javě. Jedná se o nativní schema-less grafovou databázi. Je využívána světoznámými společnostmi jako jsou Microsoft, eBay, nebo Linkedin. Klást dotazy můžeme prostřednictvím dotazovacího jazyka Cypher, který byl navržen přímo vývojáři Neo4j. Dotazy se nejběžněji píší a vyhodnocují v rozhraní nazvaném Neo4j Browser. Výsledky dotazů umožňuje zobrazit v grafové podobě, tabulce, nebo třeba JSON formátu. Při použití základní konfigurace je Neo4j Browser lokálně dostupný na adrese \url{http://localhost:7474/browser}.

Neo4j používá identifikátory s názvem \textit{elementId} pro vrcholy a hrany. Dle dokumentace však není bezpečné používat tyto identifikátory v jednotlivých transakcích a je doporučeno, pokud je jich potřeba, si vytvořit vlastní. Pro rychlý přístup k datům mají záznamy v databázi fixní délku. Jeden vrchol zabírá 15 bytů, jedna hrana 34 bytů. Na disku jsou ukládány ve spojových seznamech. Vlastnosti jsou ukládány odděleně, propojeny s vrcholy a hranami pomocí pointeru a jsou čteny jen v případě potřeby.

Neo4j lze provozovat jako samostatně spravovanou nebo cloudovou databázi. Pro první případ existují dvě varianty – Community Edition a Enterprise Edition. Community Edition je open source. Enterprise Edition se dále liší mimo jiné lepším zabezpečení, zálohováním, škálováním, podporou běhu více databází na jednom stroji a monitorováním databáze. Pro cloudové nasazení nabízí Neo4j kompletně automatizovaný databázový servis s názvem Neo4j AuraDB, spuštěný v roce 2019. AuraDB je dostupný i v bezplatné verzi, která je však omezena na 200 tisíc vrcholů a 400 tisíc hran.
...
Data jsou ukládána ve známém formátu JSON. Nejmenší jednotkou v databázi je \textit{dokument} -- jeden záznam dat. Dokumenty jsou ukládány v binární podobě zvané \textit{VelocyPack}. Dokumenty jsou dále organizovány do \textit{kolekcí}, které mohou obsahovat libovolné množství dokumentů. Každý dokument patří právě do jedné kolekce. Každý dokument má dále \textit{atributy}, které slouží jako vlastnosti. Každý dokument navíc obsahuje minimálně 3 povinné atributy, nazývané \textit{systémové}. Jsou odlišeny od ostatních tak, že začínají podtržítkem. Mají datový typ \texttt{string} a všechny jsou určené pouze ke čtení.
\begin{itemize}
...
Pro vyzkoušení dotazů můžeme využít předpřipravené csv soubory obsahující data pro naši aplikaci. Seznam všech souborů, rozdělený na entity a vztahy mezi nimi a počet řádků (včetně hlavičky) jednotlivých souborů lze vyčíst z tabulky \ref{tab:TravelEnthusiastNetwork-csv}.
...
\begin{itemize}
  ...
  \item Najdi mi všechny uživatele, s kterými podnikl cestu uživatel X.
  \item Najdi destinace nabízející některé z cestovatelských aktivit, které se uživateli líbí. Seřaď je na základě hodnocení.
  \item Pro konkrétní destinaci vrať měsíc v roce, ve kterém je nejlépe hodnocená.
...
Často při vyvíjení datového modelu vývojáři aplikace kreslí na tabuli, jak jsou data v rámci projektu mezi sebou propojena. Proto konceptuální model je v některých zdrojích označován jako \textit{whiteboard model}. 
...
\begin{center}
	...
	\caption[Grafový model pro ukázkové příklady.]{Logiký grafový model pro ukázkové příklady v této kapitole.}
...
\begin{figure}[h]
	...
	\caption[Algoritmus prohledávání do hloubky.]{Průchod algoritmu  BFS grafem.}
	\label{fig:bfs}
\end{figure}
...
Často při průchodu grafem předem známe grafový vzor určující cestu, kterou musíme projít, abychom dosáhli požadovaných výsledků pro daný dotaz. Předem víme délku cesty a vrcholové a hranové labely, přes které cesta prochází. Tento průchod grafem budeme označovat jako \textit{předdefinovaná cesta}. Při překladu otázek na~dotazy v naší aplikaci pro cestovatelské nadšence s logickým datovým modelem na~obrázku \ref{fig:Schema} zjistíme, že všechny grafové vzory z těchto otázek odvozené představují předdefinovanou cestu.
...
Problém SSSP v neohodnoceném grafu lze vyřešit pomocí algoritmů na prohledávání grafů. Konkrétně se řeší pomocí prohledávání do šířky, neboť systematicky prochází všechny vrcholy, které mají vzdálenost od počátečního vrcholu rovnou $ k $, kde $ k \in \mathbb{N}_0 $. Začíná pro $ k = 0 $ a postupně $ k $ zvětšuje o 1. V každém kroku kontroluje všechny vrcholy $u$, které mají z počátečního vrcholu vzálenost rovnou $k$. Pokud narazí na cílový vrchol, vrací $k$, které představuje délku nejkratší cesty do daného vrcholu. Algoritmus si pro každý navštívený vrchol může pamatovat vrchol, ze kterého přišel. Tímto způsobem může vrátit i konkrétní cestu, kterou se do vrcholu dostal. Nejkratších cest mezi dvěma vrcholy může být více. Pro libovolné $ m \in \mathbb{N} $ můžeme takto postupovat i pro nalezení $ m $ různých nejkratších cest pro dva dané vstupní vrcholy.
...
Najít nejkratší cestu v ohodnoceném grafu lze díky udržování a aktualizování nejkratších nalezených cest do již navštívených vrcholů. Tento princip je známý z významného algoritmu pro řešení SSSP -- Djikstrova algoritmu. Je ho využito ve zdrojovém kódu \ref{kod:ShortestPath-Weighted-Example} v jazyku Gremlin z knihy \cite{practitioner}. Dotaz najde nejkratší cestu mezi dvěma vrcholy v ohodnoceném grafu a vrátí jména stanic na této cestě, jejich počet a délku této cesty.
...
Při psaní této podkapitoly jsem čerpal též ze zdroje \cite{astar}. A* pracuje podobně jako Djikstrův algoritmus. Avšak často dosahuje výsledku rychleji díky dodatečné informaci, kterou dostane na vstupu. Této informaci se říká heuristika. Je to funkce $h\colon V\to \mathbb{R}^+$ odhadující délku cesty ze všech vrcholů do cílového vrcholu. Pro správný výsledek by heuristika měla být optimistická, tedy odhad vzdálenosti by měl být nejvýše roven skutečné vzdálenosti. Platí, že čím lépe odhadneme heuristiku tím rychleji algoritmus najde řešení. Djikstrův algoritmus je speciálním případem A*. Pro něj platí $h(v)=0$ pro všechny vrcholy $v \in V$. Pro svůj rychlý výpočet je například implementován ve videohrách.
...
V sociálních sítích mívají vrcholy i stupně v řádu stovek milionů. Tyto vrcholy představují v grafu superuzly. Abychom prošli všechny jejich sousedy, potřebujeme značně více času, než v případě \uv{obyčejného} vrcholu. To se negativně projevuje v časové odezvě u dotazů pracujících s takovými vrcholy. Degree centrality nám umožňuje takové vrcholy odhalit.
...
Algoritmus pro výpočet betweenness centrality nejprve spočítá nejkratší cesty mezi všemi páry různých vrcholů v grafu. Každý vrchol poté obdrží skóre, které je tím vyšší, čím více nejkratších cest vede přes daný vrchol. Tyto hodnoty mohou být klíčové například v grafu, kde vrcholy reprezentují křižovatky a hrany představují cesty mezi nimi. Informace o tom, kterými křižovatkami prochází nejvíce nejkratších cest, můžeme využít pro odhad lokací, kde se pravděpodobně bude pohybovat nejvíce lidí.

Algoritmy pro closeness a betweeness centrality mohou být v případě velkých grafů výpočetně dost náročné. Proto se často přistupuje k jiným, ne zcela přesným, zato mnohem rychlejším, algoritmům.

\subsubsection{PageRank}
S tímto iterativním algoritmem přišla na trh společnost Google. Byl určen pro hodnocení významnosti webových stránek, kde webové stránky si lze představit jako vrcholy grafu. Hrany mezi nimi reprezentují odkazy na stránkách odkazující na jiné stránky. Důležitost vrcholu je určena nejen důležitostí vrcholu samotného, ale také důležitostí vrcholů na tuto stránku odkazujících. Vysoké skóre vrcholu tedy znamená, že je vrchol propojen s vrcholy, které mají též vysoké skóre.

Toto řešení je založeno na vlastních vektorech a stochastických maticích z lineární algebry. Výpočet skóre většinou vede na nekonečný výpočet a proto se omezuje počet iterací tohoto výpočtu, nebo se udává maximální možná odchylka mezi výsledky iterací. PageRank se používá pro orientované grafy. Pro neorientované grafy lze použít \textit{eigenvector centrality}.
...
Při psaní této podkapitoly jsem čerpal též ze zdroje \cite{graphAlgorithms} a \cite{neo4jDoc}. Algoritmy detekce komunit v grafech se zaměřují na nalezení skupin vrcholů, tzv. \textit{komunit}, které jsou mezi sebou silněji propojeny než s vrcholy v jiných částech grafu. Příkladem využití je analýza sociálních sítí, kde se hledají komunity blízkých přátel, které tvoří shluky propojených vrcholů. Následně lze z těchto informací odhadovat chování jednotlivců podle členů jejich komunity. Znalost komunit lze též využít pro doporučování produktů uživatelům s podobným nákupním chováním.
...
\item Každá nalezená komunita tvoří vrchol v novém grafu. Ohodnocené hrany v novém grafu představují jak jsou komunity mezi sebou propojeny. Hrana propojující dva nově vzniklé vrcholy má váhu, která je součtem vah hran mezi komunitami, které tyto vrcholy reprezentují. Tento nový graf bude vstupem v další iteraci.
\end{enumerate}
Algoritmus iteruje tak dlouho, dokud dochází ke zvyšování modularity ve vzniklých komunitách. Výsledkem je rozřazení vrcholů do jednotlivých komunit na základě výsledného grafu.
...
\subsubsection{OrientDB}
Při vkládání a mazání byly mnou naměřené výsledky i více než 14krát horší než u dalších databází. Přitom bylo dodrženo doporučení z dokumentace -- hodnota WAL nastavena na hodnotu \texttt{false}. Mnou naměřené výsledky databází OrientDB a Neo4j odpovídají výsledkům experimentu \cite{experimentalComparision}.
