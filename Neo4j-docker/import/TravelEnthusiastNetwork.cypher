...
FROM 'file:///TravelEnthusiastNetwork/users.csv' AS row
...
LOAD CSV WITH HEADERS
FROM 'file:///TravelEnthusiastNetwork/journeys.csv' AS row
...
LOAD CSV WITH HEADERS
FROM 'file:///TravelEnthusiastNetwork/likes.csv' AS row
MATCH (u:User {userId: toInteger(row.userId)})
MATCH (a:TravelActivity {activityId: toInteger(row.activityId)})
MERGE (u)-[:LIKES]->(a);

LOAD CSV WITH HEADERS
FROM 'file:///TravelEnthusiastNetwork/offers.csv' AS row
MATCH (d:Destination {destinationId: toInteger(row.destinationId)})
MATCH (a:TravelActivity {activityId: toInteger(row.activityId)})
MERGE (d)-[:OFFERS]->(a);

LOAD CSV WITH HEADERS
FROM 'file:///TravelEnthusiastNetwork/participated.csv' AS row
MATCH (u:User {userId: toInteger(row.userId)})
MATCH (j:Journey {journeyId: toInteger(row.journeyId)})
MERGE (u)-[:PARTICIPATED]->(j);

LOAD CSV WITH HEADERS
FROM 'file:///TravelEnthusiastNetwork/wrote_about.csv' AS row
MATCH (u:User {userId: toInteger(row.userId)})
